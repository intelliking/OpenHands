"""Tests for the skills API endpoint (GET /api/skills).

Verifies that:
1. The endpoint returns a list of skills from global and user directories
2. Skills are sorted by source (global first) then by name
3. The endpoint handles missing directories gracefully
"""
...
async def test_skills_endpoint_returns_skills(test_client):
    ...
    assert len(data['skills']) >= 2  # At least one from each source type call
